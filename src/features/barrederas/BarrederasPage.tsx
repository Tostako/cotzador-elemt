...
/** Producto del catálogo → material de barrederas. La info del material va
 *  dentro de `description` (así la escribe guardarEnCatalogo) y el precio sale
 *  de la ferretería más barata. */
const desdeCatalogo = (p: ProductoCatalogo): Material => {
  ...
  return {
    ...nuevoMaterial(),
    id: 'cat_' + p.id,
    nombre: nombre || 'Barredera',
    color,
    tipo: d.tipo || 'aluminio',
    altura: d.altura || 10,
    modo: d.porMetro ? 'metro' : 'ceramica',
    precio_por_metro: d.porMetro ? precio : 0,
    largo_cm: d.largo_cm || 60,
    piezas_por_ceramica: d.piezas,
    precio_por_ceramica: d.porMetro ? 0 : precio,
  };
};
...
function BarrederasCalc({ planId }: { planId: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const prods = await productosDeCategoria('Barrederas');
